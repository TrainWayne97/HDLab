...
- [Syntax](#syntax)
  - [Exkurs: Die "Highs and Lows" des Computers](#exkurs-die-highs-and-lows-des-computers)
  ...
  - [Speichern von Daten: Register](#speichern-von-daten-register)
  ...
  - [Arrays von Registern](#arrays-von-registern)
...
- [SystemVerilog Erweiterung](#systemverilog-erweiterung)
  - [Praxis 2](#praxis-2)
    - [Datenspeicher 1: SR-Latch DASSELBE WIE UNTEN](#datenspeicher-1-sr-latch-dasselbe-wie-unten)
    - [Datenspeicher 2: D-Latch MUSS IM SYNTHESETOOL GETESTET WERDEN. ES KANN SEIN DASS D-LATCHES IN VERILOG ZU FEHLERN FÜHREN. \<-- Ansonsten in Theorie rein mit tollem Bild](#datenspeicher-2-d-latch-muss-im-synthesetool-getestet-werden-es-kann-sein-dass-d-latches-in-verilog-zu-fehlern-führen----ansonsten-in-theorie-rein-mit-tollem-bild)
...
- Zusatz: Versuchen Sie selbst nochmal das **XOR** und **NOR** zu bauen.
...
- Zusatz: Versuchen Sie wieder diese komplexeren Operationen aus einfachen Gattern zu bauen. Nutzen Sie ruhig alle bisher Behandelten, inklusive or und xor.
...
- Hierbei hat immer das **erste if priotität** und es wird aus einem if Block nur eine Anweisung ausgeführt.
...
- **Achtung: Das Synthesetool ist erbarmungslos! Wenn die Bitbreiten nicht passen, dann wird radikal abgeschnitten oder mit Nullen gefüllt. Meist gibt es eine Warnung, aber es ist immer gut vorsichtig zu sein, sodass das gewollte verhalten ensteht.**
...
- **WICHTIG: Wenn nur ein Wert in einer Abfolge von Operationen unsigned ist, dann wird die komplette Abfolge in der Regel als unsigned betrachtet.**
...
selektieren einzelner bits aus leitungen
zusammensetzen von Leitungen zu neuer Leitung
auffüllen von Leitungen zu 32 bits etc
Modulation also mehrere Module zu top
Unique SystemVerilog Eigenheiten
InOut
Wie funktioniert eigentlich ein FPGA - LUT DPS
Optimierung des Codes
Was sind Synthese-Tools
Was ist der unterschied zwischen synthese implementation bitstream generierung modellierung
bitbreite
Komplexere Operatoren: if else
Schleifen für Synthese
Arrays
Testbenches
CmodA7 inactive LED

EINBAUEN WIE DIE DEN CODE TESTEN KÖNNEN!!! oder Testbenches am Anfang vorfertigen --> Vllt sogar erstmal die Testbench verstecken und ein Tool drum herum anpassen, sodass man nur den Code hochlädt und dann ein Richtig oder falsch bekommt... Es sind einfache deterministische Systeme und die Ports sollen sie ja gar nicht umbennenen, bzw. kann man dafür ja nen check einbauen, der die ersten Zeilen bis zum Semikolon scannt.

(Vllt Digital logic Sim einbauen oder ähnliches, fand ich ziemlich cool zum lernen für Gatter, aber ist halt nochmal extra tool...) <-- glaube eher schlecht, weil es zielt ja alles auf verilog und nicht auf gatter, aber testen geht in dem tool direkt

Sagen dass Klammern für Lesbarkeit toll sind, bzw. eigener Teil für Lesbarkeit/Synthesefähigkeit
Vllt Truth Tables hinzufügen? --> Vllt unübersichtlich aber könnten besser erklären was zu tun ist, vor allem bei den Praxisbeispielen

Code Zeilen als Verilog, da Systemverilog die nicht so schön färbt in der Ausgabe. Warscheinlich easy fix aber naja.
...
Auslagerung
### Praxis 2
#### Datenspeicher 1: SR-Latch DASSELBE WIE UNTEN
...
#### Datenspeicher 2: D-Latch MUSS IM SYNTHESETOOL GETESTET WERDEN. ES KANN SEIN DASS D-LATCHES IN VERILOG ZU FEHLERN FÜHREN. <-- Ansonsten in Theorie rein mit tollem Bild
